...
| List all contacts (keyset paginated)            | `GET /v1/clients?cursor=&limit=`                                                    |
...
`GET /v1/clients?limit=500`. Keyset pagination — the response's
`data.nextCursor` is passed back as `cursor` to fetch the next page;
iteration ends when `nextCursor` is `null`. `limit` max 5000.
Chat-having contacts come first, then ghost contacts (created via API /
bot-assign that haven't messaged) fill the remainder; the phase
transition is tracked inside the cursor, so you don't need to handle it
manually.

Full-text contact search is intentionally not supported on this
endpoint — use `GET /v1/clients/:clientWaNumber` for exact lookup.
