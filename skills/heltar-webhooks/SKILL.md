...
Configuration calls require Bearer API key. See [`heltar-authentication`](../heltar-authentication/SKILL.md). Webhook deliveries themselves are **inbound** to your endpoint — Heltar does not send your API key and there is no signature header. To authenticate deliveries, register static `headers` on the webhook (e.g. `"headers": {"X-My-Token": "secret"}`) — they are sent on every raw-payload delivery (`metaWebhooks`, `metaCustomFieldHook`; the CleverTap/WebEngage/MoEngage converted formats don't include them), alongside an `X-Request-Timestamp` (ISO 8601). Header values come back masked on reads; resend a `*`-prefixed value to keep the stored secret. If you supplied a `verifyToken`, validate it on the GET handshake.
...
Make handlers **idempotent and order-tolerant** — failed deliveries (network errors, timeouts, `5xx`; `4xx` is never retried) are retried up to 5 times with randomized increasing delays, from a few minutes up to a few hours, so a retried event can land long after newer ones. Each attempt times out after 10 s. Meta itself can also deliver the same event more than once.
...
- Sustained consecutive delivery failures auto-disable the webhook (warning email first, disabled around 100 failures; any success resets the counter). Re-enable via the configure endpoint.
